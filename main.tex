...
deployments) with a set of detailed classifications. 
...
paid on them, which I call ``\textbf{scalability bugs}''

Cloud-scale distributed systems surpass the limit of a single machine in meeting
users' increasing demands of compute and storage, yet they has given birth to a
new type of bug: \textbf{scalability bugs}. They are latent bugs that are
...
availability at scale. And I found not much attention paid to them.

To address this new type of bugs, I started a scalability checking project. In
this project, I introduced \sck, \textit{a methodology that enables developers
to scale-check distributed systems and find scalability bugs on \textbf{one
machine}}. With \sck, I can reproduce 6 scalability bugs in Cassandra, Riak,
and Voldemort. These bugs manifest in large scale only (\eg 200 - 500 machines).
